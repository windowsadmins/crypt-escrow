...
if ($Deploy -and [string]::IsNullOrWhiteSpace($CryptServer)) {
    ...
    Write-Host "Example: .\build.ps1 -Deploy -CryptServer https://crypt.example.com" -ForegroundColor Yellow
    exit 1
}
...
function Get-SigningCertThumbprint {
    ...
    $cert = Get-ChildItem Cert:\CurrentUser\My -ErrorAction SilentlyContinue | 
        Where-Object { $_.HasPrivateKey -and $_.Subject -like '*ExampleOrgU*' -and $_.NotAfter -gt (Get-Date) } |
...
function signNuget {
    ...
    & nuget.exe sign `
        ...
        -CertificateSubjectName 'ExampleOrgU Intune Windows Enterprise Certificate' `
        -Timestamper $tsa
    if ($LASTEXITCODE) {
...
# Test connectivity
Test-NetConnection crypt.example.com -Port 443
